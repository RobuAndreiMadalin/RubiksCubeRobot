...
// Implementarea constructorului clasei MyServo
MyServo::MyServo(int pin, ServoType type, ServoCal calibration) :
    pin(pin),
    type(type),
    cal(calibration)
{
    // Spinnerii (index par) stau la Centru implicit, Sliderii (index impar) stau la R (Release)
...
void MyServo::setState(ServoState next) {
    switch (next) {
        ...
        case STATE_C:
            if (static_cast<int>(type) % 2 == 1) { // Slider
                pulse = cal.C_us;
            } else { // Spinner - compensare pentru jocul gripei
